#!/usr/bin/env python3
"""tof.py — ToF 3개 기반 Assisted Teleop 제어 연산 (라파2).
...
class Controller:
    def __init__(self, cfg):
        ...
        self._side_min = cfg['side_min_distance']
        self._side_clear = cfg['side_min_distance'] + cfg['side_clear_margin']
...
class SysMonitor(threading.Thread):
    ...
    def run(self):
        while not self._stop.is_set():
            ...
            me = self._proc_stat('tof.py', os.getpid(), now)
            procs['tof.py'] = me
...
function renderSys(sys, hz){
  ...
  cardsEl.innerHTML =
    proc("tof.py", "tof.py (제어)", ` · ${hz ? hz.toFixed(1) : "?"}Hz`) +
...
def main():
    ...
    while running['v']:
        ...
        for tag, reader in readers.items():
            ...
            # ── 장애물: ROI 최소거리 ──────────────────────────────────────
            if fresh:
                vals, _ = roi_values(grid, obs_rows, obs_cols)
                d = (min(vals) / 1000.0) if vals else None
            else:
                d = None
