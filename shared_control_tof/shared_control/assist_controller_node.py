...
핵심 알고리즘 (프롬프트 + 개선안):
    ...
    elif 좌·우 모두 side_min(150mm) 이내:
        if 정면도 막힘(<=front_stop 200mm): → STOP      (양쪽+정면 막힘 → 갈 곳 없음)
        else:                            → 직진 통과    (정면 뚫림 → 회전 없이 그대로 전진)
...
class AssistControllerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('side_min_distance', 0.15)       # m (150mm)
        self.declare_parameter('side_clear_margin', 0.05)       # m  해제 여유
        # 전방이 이 값 이하이면 '완전 막힘'으로 보고 전진을 멈춘다(제자리 회전 탐색).
        # 양쪽 측면도 함께 막혀 있으면 갈 곳 없음 → 정지.
        self.declare_parameter('front_stop_distance', 0.20)     # m (200mm)
        # 회피 속도 (비례 조향)
    ...
    def _tick(self):
        ...
        # 정면 '완전 막힘': front_stop(200mm) 이하 → 직진으로도 빠져나갈 수 없음.
        front_fully_blocked = self._front <= self._front_stop
        ...
        if not engaged:
            ...
            # 양쪽 벽 모두 side_min(150mm) 이내 → 좌·우로는 꺾을 수 없음.
            if front_fully_blocked:
                # 정면도 막힘(<=200mm) → 갈 곳 없음 → 정지
                state = STATE_STOP
            else:
                # 정면은 뚫림(>200mm) → 회전 없이 그대로 직진 통과(좁은 통로).
                #   사용자 전진 의도를 살리되 v_avoid 로 속도만 제한(안전).
                state = STATE_NORMAL
                out.linear.x = min(manual.linear.x, self._v_avoid)
                out.angular.z = 0.0
